...
void setup () {
  size(700,700);
  output = createWriter("positions2.txt"); 
  smooth();
  background(162);
  textSize(16);
  ...
  for (int x = 0; x < 5; x++) Units.add(new Unit(20, height -30 + x));
  //for (int x = 1; x < 5; x++) Units.add(new Unit(20, height -30 + x)); 
  //for (int x = 1; x < 5; x++) Units.add(new Unit(20, height -30 + x)); 
  setupWalls();
  setupScenario_2();
}
...
void draw () {
  ...
  for (int i = 0; i <Units.size(); i++) {
    ...
    System.out.println("Unit"+i+":{ X = "+current.pos.x+"; Y = "+current.pos.y+"; "+(System.currentTimeMillis()-start));
    output.println("Unit "+i+";"+current.pos.x+";"+current.pos.y+"; "+(System.currentTimeMillis()-start));
    current.go();
    current.draw();
